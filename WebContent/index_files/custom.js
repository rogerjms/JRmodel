	...
	if( jQuery().validate ){
		...
		$("#appoint-form").validate({
			...
			submitHandler: function(form) {
				$.ajax({
				   type: "POST",
				   url: "user.do?method=checkaccountno",
				   data: "accountno="+$("#apo_name").val()+"&safecode="+$("#safecode").val(),
				   async: false,
				   success: function(msg){
					 var result = $.parseJSON( msg );
					 
					 if(result.safecode.trim()=="true"){
						 if(result.accountno.trim()=="true"){
							 $(form).ajaxSubmit(appointment_options);
						 }else{
							 $("#appoint-form .error-container")
								.html("")
								.html("<label class='error' style='display: block;'> 用户名已存在 </label>").show();
							 $("#apo_name").focus();
						 }
					 }else{
						 $("#appoint-form .error-container")
							.html("")
							.html("<label class='error' style='display: block;'> 验证码填写错误，请检查验证码 </label>").show();
						 $("#safecode").focus();
					 }
				   }
				});
